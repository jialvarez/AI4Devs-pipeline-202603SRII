...
# Prompt 6 — Fix: @prisma/client no inicializado en producción
Tras el primer despliegue real, `pm2 list` mostraba el proceso `backend` en estado `errored` y `pm2 logs backend` mostraba `Error: @prisma/client did not initialize yet. Please run "prisma generate" and try to import it again.`. Causa: el job `build` solo empaquetaba `dist/`, `package.json` y `package-lock.json`, sin `prisma/schema.prisma`, y el job `deploy` nunca ejecutaba `prisma generate` en el servidor (el cliente de Prisma se genera a partir del schema y no viaja con `npm ci`). Fix aplicado en `ci.yml`: se añadió `backend/prisma` al `path` del artefacto subido en el job `build`, y se añadió el paso `npx prisma generate` en el script SSH del job `deploy`, justo después de `npm ci --omit=dev` y antes del `pm2 restart`.